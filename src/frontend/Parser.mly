...
ty:
    ...
    // | TBIT single_poly                  { ty_sp (TBIT (snd $2)) (Span.extend $1 (fst $2)) }
    // | TBIT                              { ty_sp (TBIT (IConst 32)) $1 }
    | TBOOL				                      { ty_sp TBool $1 }
    ...
    | tyconstrs                           { ty_sp (TConstrs (snd $1)) (fst $1) }
...
tyconstr:
    | ID                                { (fst $1, Id.name (snd $1)) }

tyconstrs:
    | tyconstr PIPE tyconstr				{ Span.extend (fst $1) (fst $3), [snd $1; snd $3] }
    | tyconstr PIPE tyconstrs               { Span.extend (fst $1) (fst $3), (snd $1)::(snd $3) }
...
num_numw:
    | NUMWITDH                            { fst (snd $1), (Some (IConst (snd (snd $1)))) }
    | NUM                                 { snd $1, None }

pattern:
    | cid                               { pat_of_cid $1 }
    | num_numw                          { PNum (fst $1, snd $1) }
    | BITPAT                            { PBit (snd $1) }
    | num_numw PRANGE num_numw          { PRange ($1, $3) }
...
exp:
    ...
    | exp IF exp ELSE exp               { ternary_sp $3 $1 $5 (Span.extend $1.espan $5.espan) } 
...
statement1:
    ...
    | ty cid ASSIGN exp SEMI                 { slocal_sp (snd $2) $1 $4 (Span.extend $1.tspan $5) }
    | NOINLINE ty cid ASSIGN exp SEMI        { slocal_sp (snd $3) $2 $5 (Span.extend $1 $6) |> noinline }
    | cid ASSIGN exp SEMI	                { sassign_sp (snd $1) $3 (Span.extend (fst $1) $4) }
    | NOINLINE cid ASSIGN exp SEMI	        { sassign_sp (snd $2) $4 (Span.extend $1 $5) |> noinline }
