...
let allowed_bool_op = function
  | Eq | Neq | Less | More | And | Or | Not ->
      true
  | _ ->
      false

let allowed_int_op = function Plus | Sub | BitAnd | BitOr -> true | _ -> false
...
let extract_simple_body mem1 local1 body =
  ...
  | [{s= SIf (e, s1, s2)}] -> (
      ...
      | _ ->
          error_sp body.sspan
            "Invalid if statement in a memop with two arguments" )
  | _ ->
      error_sp body.sspan "Invalid form for a memop with two arguments"

(* Simplified representation of the statements in the body, as an intermediate form
   while putting together a complex_body *)
type complex_stmt =
  | BoolDef of cid * exp
  | CellAssign of cid * conditional_return option * conditional_return option
...
let classify_stmts (body : statement) : (complex_stmt * sp) list =
  let classify s =
    let ret =
      match s.s with
      | SLocal (id, {raw_ty= TBool}, e) ->
          BoolDef (id, e)
      | SUnit {e= ECall (cid, es)} ->
          ExternCall (cid, es)
      | SIf (test1, s1, s2) -> (
        match (flatten_stmt s1, flatten_stmt s2) with
        | [{s= SRet (Some ret)}], [] ->
        ...
        | [{s= SAssign (id1, ret1)}], [{s= SIf (test2, s1', s2')}] -> (
          match (flatten_stmt s1', flatten_stmt s2') with
          | [{s= SAssign (id2, ret2)}], [] ->
              if not (Cid.equal id1 id2) then
                error_sp s1.sspan
                @@ "If a complex memop has an if/else, both parts must assign \
                    to the same cell variable, not " ^ Cid.to_string id1
                ^ " and " ^ Cid.to_string id2 ;
              CellAssign (id1, Some (test1, ret1), Some (test2, ret2))
          | _ ->
              error_sp s.sspan "Invalid if statement in complex memop" )
        | _ ->
            error_sp s.sspan "Invalid if statement in complex memop" )
      | _ ->
          error_sp s.sspan "Invalid statement type in complex memop"
    in
    (ret, s.sspan)
  in
  ...
  List.iter
    (function
      | BoolDef _, _ | LocalRet _, _ | ExternCall _, _ ->
          ()
      | CellAssign (id, _, _), sp when Cid.equal id cell1_id ->
          if fst !counts then
            error_sp sp @@ Cid.to_string cell1_id
            ^ " is assigned to in multiple parts of this memop" ;
          counts := (true, snd !counts)
      | CellAssign (id, _, _), sp when Cid.equal id cell2_id ->
          if snd !counts then
            error_sp sp @@ Cid.to_string cell2_id
            ^ " is assigned to in multiple parts of this memop" ;
          counts := (fst !counts, true)
      | CellAssign (id, _, _), sp ->
          error_sp sp @@ Cid.to_string id
...
let extract_complex_body mems locals body =
  ...
  let check_assign_cr cro =
    let check (cond, e) = check_cond cond ; check_int e in
  ...
  let check_return_cr (cond, e) =
    ...
    match e.e with
    | EVar (Id id)
      when Cid.equal (Id id) cell1_id || Cid.equal (Id id) cell2_id
  ...
  let cell1, cell2, body =
    ...
    | (CellAssign (id1, cr1_1, cr1_2), _)
      :: (CellAssign (_, cr2_1, cr2_2), _) :: tl ->
        ...
        if Cid.names id1 = Cid.names cell1_id then
          ((cr1_1, cr1_2), (cr2_1, cr2_2), tl)
        else ((cr2_1, cr2_2), (cr1_1, cr1_2), tl)
    | (CellAssign (id1, cr1_1, cr1_2), _) :: tl ->
        List.iter check_assign_cr [cr1_1; cr1_2] ;
        if Cid.names id1 = Cid.names cell1_id then
          ((cr1_1, cr1_2), (None, None), tl)
        else ((None, None), (cr1_1, cr1_2), tl)
    | _ ->
        ((None, None), (None, None), body)
  in
  let extern_calls, body =
    List.span (function ExternCall _, _ -> true | _ -> false) body
  in
  let extern_calls =
    List.map
      (function
        | ExternCall (cid, es), _ -> (cid, es) | _ -> failwith "impossible" )
      extern_calls
  in
  let ret =
    match body with
    | [] ->
        None
    | [(LocalRet cr, _)] ->
        check_return_cr cr ; Some cr
    | (_, sp) :: _ ->
        error_sp sp
        @@ "Unexpected statement in a memop. Are you sure your statements are \
            in order?"
  in
...
let ensure_same_size span params =
  if List.is_empty params then
    Console.error_position span "A memop cannot have 0 arguments!" ;
  let sizes =
    List.map
      (fun (_, ty) ->
        ...
        | TInt sz ->
            sz
        | _ ->
            Console.error_position ty.tspan
              "All arguments to a memop must be integers" )
      params
  in
  if not (List.for_all (equiv_size (List.hd sizes)) sizes) then
...
let extract_memop span (params : params) (body : statement) : memop_body =
  ...
  List.iter
    (fun (id, _) ->
      if Cid.equal (Id id) cell1_id || Cid.equal (Id id) cell2_id then
        error_sp span "Arguments to a memop may not be named cell1 or cell2" )
    params ;
  match params with
  | [(mem1, _); (local1, _)] ->
      extract_simple_body mem1 local1 body
  ...
  | _ ->
      error_sp span "A memop must have exactly 2, 3, or 4 arguments"
