...
let unwrap_effect eff =
  let rec aux eff =
    ...
    | FZero ->
        (FZero, [(None, 0)])
    | FVar (QVar _) | FVar (TVar {contents= Unbound _}) ->
        (eff, [(None, 0)])
    | FVar (TVar {contents= Link eff}) ->
        aux eff
    | FSucc eff ->
  ...
  | (None, hd) :: tl ->
      List.fold_left
        (fun acc (o, n) ->
          match o with
          | None ->
              add_succs (FProj acc) n
          | Some id ->
              add_succs (FIndex (id, acc)) n )
        (add_succs base hd) tl
  | (Some _, _) :: _ ->
      failwith "First element of list cannot be FIndex!"
...
let rec is_global_rty rty =
  ...
  | TConstrs _ ->
      false (* Molly thinks *)
  | TName (_, _, b) | TAbstract (_, _, b, _) ->
      b
  | TTuple lst ->
      List.exists is_global_rty lst
  | TRecord lst ->
      List.exists (fun (_, rty) -> is_global_rty rty) lst
  | TVector (t, _) ->
      is_global_rty t
  | TTable _ ->
      true
...
let rec is_not_global_rty rty =
  ...
  | TName (_, _, b) | TAbstract (_, _, b, _) ->
      not b
  | TTuple lst ->
      List.for_all is_not_global_rty lst
  | TRecord lst ->
      List.for_all (fun (_, rty) -> is_not_global_rty rty) lst
  | TVector (t, _) ->
      is_not_global_rty t
  | TTable _ ->
      false
...
let add_sizes s1 s2 =
  ...
  | IConst n1, IConst n2 ->
      IConst (n1 + n2)
  | ISum (ss, n2), IConst n1 | IConst n1, ISum (ss, n2) ->
      ISum (ss, n1 + n2)
  | ISum (ss, n), ((IVar _ | IUser _) as s)
  | ((IVar _ | IUser _) as s), ISum (ss, n) ->
      ISum (s :: ss, n)
  | IConst n1, ((IVar _ | IUser _) as s) | ((IVar _ | IUser _) as s), IConst n1
    ->
      ISum ([s], n1)
  | (IVar _ | IUser _), (IVar _ | IUser _) ->
      ISum ([s1; s2], 0)
...
let rec normalize_size s =
  ...
  | ISum (vs, n) -> (
      ...
      | ISum ([], _) ->
          failwith "Sanity check: this should never happen"
      | ISum ([s], 0) ->
          s
      | ISum (vs, n) ->
          ISum (List.sort Stdlib.compare vs, n)
      | sz ->
          sz )
  | s ->
      s
...
let extract_size_default s def =
  match normalize_size s with IConst n -> n | _ -> def
...
let extract_size s =
  ...
  | IConst n ->
      n
  | _ ->
      failwith "[extract_size] error: normalized size is not a const"
...
let rec equiv_lists f lst1 lst2 =
  ...
  | [], [] ->
      true
  | hd1 :: tl1, hd2 :: tl2 ->
      f hd1 hd2 && equiv_lists f tl1 tl2
  | _ ->
      false
...
let rec equiv_size ?(qvars_wild = false) s1 s2 =
  ...
  | IUser id1, IUser id2 ->
      Cid.equal id1 id2
  | ISum (vs1, n1), ISum (vs2, n2) ->
      n1 = n2 && equiv_lists equiv_size vs1 vs2
  | IConst n1, ISum (vs, n2) | ISum (vs, n2), IConst n1 ->
      (* Special case *)
      qvars_wild && n2 <= n1
      && List.for_all (function IVar (QVar _) -> true | _ -> false) vs
  | IVar tqv, s | s, IVar tqv ->
      STQVar.equiv_tqvar ~qvars_wild equiv_size tqv s
  | _ ->
      false
...
let try_subtract_sizes s1 s2 =
  match (normalize_size s1, normalize_size s2) with
  | IConst n, IConst m when n >= m ->
      Some (IConst (n - m))
  | ISum (sizes, n), IConst m when n >= m ->
      Some (ISum (sizes, n - m))
...
let rec equiv_effect ?(qvars_wild = false) e1 e2 =
  ...
  | FZero, FZero ->
      true
  | FSucc e1', FSucc e2' | FProj e1', FProj e2' ->
      equiv_effect e1' e2'
  ...
  | (FZero | FSucc _ | FProj _ | FIndex _), _ ->
      false
...
let equiv_constraints lst1 lst2 =
  List.for_all2
    ...
    (List.sort Stdlib.compare lst1)
    (List.sort Stdlib.compare lst2)
...
let normalizer () =
  ...
  let v =
    object (self)
      ...
      val renaming : id IdMap.t = IdMap.create 0 (* mutable *)

      method! visit_QVar _ _ id =
        match IdMap.find_option renaming id with
        | Some id' ->
            QVar id'
        | None ->
            let new_id = Id.create ("norm" ^ string_of_int !count) in
            incr count ;
            IdMap.add renaming id new_id ;
            QVar new_id
...
let rec equiv_raw_ty ?(ignore_effects = false) ?(qvars_wild = false) ty1 ty2 =
  ...
  | TInt size1, TInt size2 ->
      equiv_size size1 size2
  | TConstrs lst1, TConstrs lst2 ->
      if List.length lst1 <> List.length lst2 then false
      else List.for_all2 (fun str1 str2 -> String.equal str1 str2) lst1 lst2
  | TPat size1, TPat size2 ->
      equiv_size size1 size2
  | TMemop (n1, size1), TMemop (n2, size2) ->
      n1 = n2 && equiv_size size1 size2
...
and equiv_ty ?(ignore_effects = false) ?(qvars_wild = false) ty1 ty2 =
  ( ignore_effects || is_not_global ty1
...
let max_effect e1 e2 =
  ...
  if not (equiv_effect base1 base2) then None
...
let default_expression ty =
  let rec aux rty =
    match TyTQVar.strip_links rty with
    | TInt size ->
        eint (Z.of_int 32) (Some size)
    | TBool ->
        value_to_exp (vbool false)
    | TVector (raw_ty, size) -> (
      match size with
      | IConst n ->
          vector_sp (List.init n (fun _ -> aux raw_ty)) Span.default
      | _ ->
          comp_sp (aux raw_ty) (Id.create "_") size Span.default )
    | TRecord lst ->
        record_sp
          (List.map (fun (s, raw_ty) -> (s, aux raw_ty)) lst)
          Span.default
    | _ ->
        failwith
        @@ "Can only create default expression for types int or bool, or \
            records and vectors of such"
  in
  aux ty.raw_ty
...
let rec is_compound e =
  match e.e with
  | EInt _ | EVal _ | EVar _ | ESizeCast _ | EPatWild _ ->
      false
  | ETernary _ | EHash _ | EOp _ | ECall _ | EStmt _ ->
      true
  | ETableCreate _ ->
      true
  | ETableMatch _ ->
      true
  | EComp (e, _, _) | EIndex (e, _) | EProj (e, _) | EFlood e ->
      is_compound e
  | EVector entries | ETuple entries ->
      List.exists is_compound entries
  | ERecord entries ->
      List.exists (is_compound % snd) entries
  | EWith (base, entries) ->
...
let complex_body_to_stmt (body : complex_body) =
  let handle_bool b = Option.map (fun (cid, e) -> slocal cid (ty TBool) e) b in
  let handle_cell id (cro1, cro2) =
    let els =
      ...
      | None ->
          snoop
      | Some (cond, e) ->
          sifte cond (sassign id e) snoop
    in
    match cro1 with
    | None ->
        None
    | Some (cond, e) ->
        Some (sifte cond (sassign id e) els)
  in
...
let memop_body_to_stmt memop_body =
  ...
  | MBReturn e ->
      statement (SRet (Some e))
  | MBIf (e1, e2, e3) ->
      sifte e1 (statement (SRet (Some e2))) (statement (SRet (Some e3)))
  | MBComplex b ->
      complex_body_to_stmt b

let simplify_constraint = function
  | CLeq (e1, e2) ->
      if equiv_effect e1 e2 || e1 = FZero then Some true
      else
  ...
  let rec aux acc cs =
    ...
    | hd :: tl -> (
      ...
      | Some true ->
          aux acc tl
      | Some false ->
          if ignore_contradictions then aux (hd :: acc) tl
...
let cid_of_exp (ex : exp) : Cid.t =
  ...
  | EVar n ->
      n
  | _ ->
      error "could not evaluate expression to a cid"

let exp_to_cid = cid_of_exp

let is_evar exp = match exp.e with EVar _ -> true | _ -> false

let rec numeric_raw_ty_size (ty : raw_ty) : int =
  match ty with
  | TBool ->
      1
  | TInt (IConst i) ->
      i
  | TRecord fields ->
      sum (List.map (fun (_, ty) -> numeric_raw_ty_size ty) fields)
  | TVector (ty, IConst i) ->
      numeric_raw_ty_size ty * i
  | TTuple items ->
      sum (List.map numeric_raw_ty_size items)
  | TPat (IConst i) ->
      i
  | _ ->
      raise (Error "[SyntaxUtils] Unexpected type without numeric size.")
