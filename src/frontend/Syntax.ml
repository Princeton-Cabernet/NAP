...
and constr_spec_cmp = SpecLess | SpecLeq

and constr_spec = CSpec of (cid * constr_spec_cmp) list | CEnd of cid
...
and raw_ty =
  ...
  (* | TBit of size Number of bits *)
  | TConstrs of string list
...
and tbl_ty = {tkey_sizes: size list; tparam_tys: ty list; tret_tys: ty list}

and acn_ty = {aconst_param_tys: tys; aparam_tys: tys; aret_tys: tys}
...
and op =
  ...
  | PatExact
  | PatMask
(* x && y matches z when z && y == x && y *)

and pat =
  | PWild
  (* Span is just for easy error messaging in ConstInlining *)
  | PVar of cid * sp
  | PNum of z * size option
  | PBit of int list (* mask *)
  | PRange of (z * size option) * (z * size option)
...
and v =
  ...
  | VPat of int list
(* every pattern typed expression evaluates to a VPat *)
...
(* and tunit =
   ...
   | Min
   | Hour *)
and event = {eid: cid; data: value list; edelay: int}

and value = {v: v; vty: ty option; vspan: sp}
...
and e =
  ...
  | EPatWild of size option
(* Polymorphic wildcard pat, handled similar to EInt *)

and exp = {e: e; ety: ty option; espan: sp}
...
and tbl_entry = {eprio: int; ematch: exp list; eaction: id; eargs: exp list}
...
and interface_spec = {ispec: ispec; ispan: sp}
...
let ty_sp raw_ty tspan =
  {raw_ty; teffect= FVar (QVar (Id.fresh "eff")); tspan; tprint_as= ref None}
...
module STQVar = TQVar.Make (struct
  ...
  let proj a = match a with IVar x -> Some x | _ -> None
...
module FTQVar = TQVar.Make (struct
  ...
  let proj a = match a with FVar x -> Some x | _ -> None
...
module TyTQVar = TQVar.Make (struct
  ...
  let proj a = match a with TQVar x -> Some x | _ -> None
...
let ternary_sp cond e1 e2 span = exp_sp (ETernary (cond, e1, e2)) span
