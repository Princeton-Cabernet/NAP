...
type t = Id of id | Compound of id * t
...
let rec create ss =
  ...
  | [s] ->
      Id (Id.create s)
  | s :: ss ->
      Compound (Id.create s, create ss)
...
let rec freshen_last cid =
  ...
  | Id id ->
      Id (Id.freshen id)
  | Compound (id, cid) ->
      Compound (id, freshen_last cid)

let str_cons str cid : t =
  match cid with
  | Id id ->
      Compound (Id.fresh str, Id id)
  | Compound (id, cid) ->
      Compound (Id.fresh str, Compound (id, cid))
...
let rec to_string cid =
  ...
  | Id id ->
      Id.to_string id
  | Compound (id, cid) ->
      Id.to_string id ^ "." ^ to_string cid

let rec to_string_delim d cid =
  match cid with
  | Id id ->
      Id.to_string_delim d id
  | Compound (id, cid) ->
      Id.to_string_delim d id ^ "." ^ to_string_delim d cid
...
let to_id d =
  ...
  | Id evid ->
      evid
  | _ ->
      failwith "attempted to convert cid with multiple parts into id"

let rec to_ids d =
  match d with Id id -> [id] | Compound (id, cid) -> id :: to_ids cid
...
let rec first_id d = match d with Id id -> id | Compound (id, _) -> id

let rec last_id d =
  match d with Id id -> id | Compound (_, cid) -> last_id cid
...
let exists tuples key =
  match lookup_opt tuples key with Some _ -> true | None -> false
...
let rec replace tuples key new_val =
  ...
  | [] ->
      []
  | (k, v) :: tuples ->
      if equals k key then (k, new_val) :: tuples
...
let rec modify_tail f t =
  ...
  | Id id ->
      Id (f id)
  | Compound (id, cid) ->
      Compound (id, modify_tail f cid)

let length t = List.length (names t)

let modify_head f t =
  match t with Id id -> Id (f id) | Compound (id, cid) -> Compound (f id, cid)
