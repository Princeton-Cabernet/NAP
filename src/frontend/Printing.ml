...
let cid_to_string cid =
  if cfg.verbose_types then Cid.to_string cid
...
let tqvar_to_string tqv =
  if cfg.verbose_types then tqvar_to_string_full tqv
  else tqvar_to_string_friendly tqv

let rec size_to_string s =
  match s with
  | IVar tqv ->
      tqvar_to_string size_to_string tqv
  | IConst i ->
      string_of_int i
  | IUser cid ->
      cid_to_string cid
...
let cspecs_to_string constraints =
  let cmp_to_string cmp =
    match cmp with SpecLess -> " < " | SpecLeq -> " <= "
  in
        ...
        (function
          | CSpec lst ->
              ...
              @@ List.fold_left
                   ...
                   (List.tl lst)
          | CEnd cid1 ->
              "end " ^ cid_to_string cid1 )
        constraints

let rec raw_ty_to_string t =
  match t with
  | TQVar tqv ->
      tqvar_to_string raw_ty_to_string tqv
  | TBool ->
      "bool"
  | TInt i ->
      "int<<" ^ size_to_string i ^ ">>"
  | TConstrs lst ->
      String.concat " | " lst
  | TName (cid, sizes, b) ->
      cid_to_string cid ^ sizes_to_string sizes
      ^ if cfg.verbose_types then "{" ^ string_of_bool b ^ "}" else ""
  | TAbstract (cid, sizes, b, _) ->
      let base =
        if cfg.verbose_types then Printf.sprintf "Abs[%s]" (cid_to_string cid)
        else cid_to_string cid
      in
      base ^ sizes_to_string sizes
      ^ if cfg.verbose_types then "{" ^ string_of_bool b ^ "}" else ""
  | TEvent ->
      "event"
  | TFun func ->
      func_to_string func
  | TMemop (n, size) ->
      Printf.sprintf "memop%d<<%s>>" n (size_to_string size)
  | TVoid ->
      "void"
  | TGroup ->
      "group"
  ...
  | TTuple tys ->
      "(" ^ concat_map " * " raw_ty_to_string tys ^ ")"
  | TTable t ->
      " table_type {" ^ "\n\tkey_size: "
...
and ty_to_string t =
  let eff_str =
    if cfg.show_effects && (is_global t || cfg.show_all_effects) then
      effect_to_string t.teffect |> wrap "(" ")"
    else ""
  in
  ...
  | _ ->
      raw_ty_to_string t.raw_ty ^ eff_str

let pat_to_string p =
  let z_size_to_string z size =
    Z.to_string z
    ^
  ...
  | PWild ->
      "_"
  | PNum (z, size) ->
      z_size_to_string z size
  | PVar (cid, _) ->
      cid_to_string cid
  | PBit bs ->
      "0b"
      ^ ( bs
        |> List.map (function 0 -> '0' | 1 -> '1' | _ -> '*')
        |> String.of_list )
  | PRange ((lo_z, lo_size), (hi_z, hi_size)) ->
      z_size_to_string lo_z lo_size ^ " .. " ^ z_size_to_string hi_z hi_size
...
let rec v_to_string v =
  ...
  | VGroup vs ->
      Printf.sprintf "{%s}" (comma_sep location_to_string vs)
  | VPat bs ->
      bs_to_string bs
...
and event_to_string {eid; data; edelay} =
  let delaystr =
    if edelay <> 0 && cfg.verbose_types then "@" ^ string_of_int edelay ^ "ns"
    else ""
  in
  Printf.sprintf "%s(%s)%s" (cid_to_string eid)
...
let rec e_to_string e =
  ...
  | EInt (z, size) -> (
      ...
      | Some size ->
          "<<" ^ size_to_string size ^ ">>" )
  | EOp (op, [e]) ->
      op_to_string op ^ exp_to_string e
  | EOp (op, [e1; e2]) ->
      exp_to_string e1 ^ op_to_string op ^ exp_to_string e2
  | EOp (op, es) ->
      error
  ...
  | ETernary (cond, e1, e2) ->
      "if " ^ e_to_string cond.e ^ " then " ^ e_to_string e1.e ^ " else "
      ^ e_to_string e2.e
  | EHash (size, es) ->
      Printf.sprintf "hash<<%s>>(%s)" (size_to_string size) (es_to_string es)
  | EFlood e ->
      Printf.sprintf "flood %s" (exp_to_string e)
  | EProj (e, l) ->
      exp_to_string e ^ "#" ^ l
  | ERecord lst ->
      Printf.sprintf "{%s}"
        (concat_map "; "
           (fun (str, exp) -> str ^ " = " ^ exp_to_string exp)
           lst )
  | EWith (base, lst) ->
      Printf.sprintf "{%s with %s}" (exp_to_string base)
        (concat_map "; "
           (fun (str, exp) -> str ^ " = " ^ exp_to_string exp)
           lst )
  | EVector es ->
      list_to_string exp_to_string es
  | EComp (e, i, k) ->
      Printf.sprintf "[%s for %s < %s]" (exp_to_string e) (id_to_string i)
        (size_to_string k)
  | EIndex (e, i) ->
      Printf.sprintf "%s[%s]" (exp_to_string e) (size_to_string i)
  | ETuple es ->
      "(" ^ concat_map ", " exp_to_string es ^ ")"
  | ESizeCast (sz1, sz2) ->
      Printf.sprintf "to_int<<%s>>(%s)" (size_to_string sz1)
        (size_to_string sz2)
  | EStmt (s, e) ->
      Printf.sprintf "{%s; return %s}" (stmt_to_string s) (exp_to_string e)
  | ETableCreate t ->
      Printf.sprintf "table_create<%s>((%s),%s, %s(%s))" (ty_to_string t.tty)
...
and stmt_to_string s =
  match s.s with
  | SAssign (i, e) ->
      cid_to_string i ^ " = " ^ exp_to_string e ^ ";"
  | SNoop ->
      "skip;"
  | SGen (g, e) -> (
    match g with
    | GSingle None ->
        Printf.sprintf "generate %s;" (exp_to_string e)
    | _ ->
        let gen_str, loc =
          match g with
          | GSingle eo ->
              ("generate_switch", Option.get eo)
          | GMulti loc ->
              ("generate_ports", loc)
          | GPort loc ->
              ("generate_port", loc)
        in
        Printf.sprintf "%s (%s, %s);" gen_str (exp_to_string loc)
          (exp_to_string e) )
  | SLocal (i, t, e) ->
      Printf.sprintf "%s %s = %s;"
  ...
  | SIf (e, s1, s2) ->
      let s2_str =
        ...
        | _ ->
            "else {\n" ^ stmt_to_string s2 ^ "\n}"
      in
      Printf.sprintf "if (%s) {\n%s\n} %s" (exp_to_string e) (stmt_to_string s1)
        s2_str
  | SRet eopt ->
      let estr =
        match eopt with Some e -> " " ^ exp_to_string e | None -> ""
      in
  ...
  | SLoop (s, i, k) ->
      Printf.sprintf "for (%s < %s) {\n%s\n}" (id_to_string i)
        (size_to_string k) (stmt_to_string s)
  | STableMatch tbl_rec ->
      if tbl_rec.out_tys <> None then
        Printf.sprintf "%s %s = table_match(%s, (%s), (%s));"
        ...
        Printf.sprintf "%s = table_match(%s);"
          (comma_sep id_to_string tbl_rec.outs)
          (comma_sep exp_to_string
             ((tbl_rec.tbl :: tbl_rec.keys) @ tbl_rec.args) )
...
let event_sort_to_string sort =
  match sort with EPacket -> "entry event" | EBackground -> "event"

let rec interface_spec_to_string spec =
  match spec.ispec with
  | InSize id ->
      Printf.sprintf "size %s;" (id_to_string id)
  | InVar (id, ty) ->
  ...
  | InTy (id, sizes, tyo, b) -> (
      ...
      | None ->
          decl ^ ";"
      | Some ty ->
          decl ^ " = " ^ ty_to_string ty )
  | InConstr (id, ty, params) ->
      Printf.sprintf "constr %s %s(%s);" (ty_to_string ty) (id_to_string id)
        (params_to_string params)
  | InFun (id, rty, cspecs, params) ->
      Printf.sprintf "fun %s %s(%s)%s;" (ty_to_string rty) (id_to_string id)
        (params_to_string params) (cspecs_to_string cspecs)
  | InEvent (id, cspecs, params) ->
      Printf.sprintf "event %s(%s)%s;" (id_to_string id)
        (params_to_string params) (cspecs_to_string cspecs)
  | InModule (id, intf) ->
...
and parser_action_to_string action =
  match action with
  | PSkip ty ->
      Printf.sprintf "skip %s;" (ty_to_string ty)
  | PRead (id, ty) ->
...
and parser_step_to_string step =
  ...
  | PGen e ->
      Printf.sprintf "generate %s;" (exp_to_string e)
  | PCall e ->
      Printf.sprintf "%s;" (exp_to_string e)
  | PMatch (e, branches) ->
      Printf.sprintf "match %s with %s" (exp_to_string e)
...
and d_to_string d =
  ...
  | DHandler (id, hsort, (params, s)) ->
      Printf.sprintf "%shandle %s(%s) {\n%s\n}"
        ...
        | HEgress ->
            "@egress" )
        (id_to_string id) (params_to_string params) (stmt_to_string s)
  ...
  | DFun (id, rty, cspecs, (params, s)) ->
      Printf.sprintf "fun %s %s(%s)%s {\n%s\n}" (ty_to_string rty)
        (id_to_string id) (params_to_string params) (cspecs_to_string cspecs)
        (stmt_to_string s)
  | DMemop (id, params, mbody) ->
      Printf.sprintf "memop %s(%s)\n {%s}" (id_to_string id)
        (params_to_string params) (memop_to_string mbody)
  | DSize (id, szo) -> (
    match szo with
    | None ->
        Printf.sprintf "size %s;" (id_to_string id)
  ...
  | DConst (id, ty, e) ->
      Printf.sprintf "const %s %s = %s;" (id_to_string id) (ty_to_string ty)
        (exp_to_string e)
  | DExtern (id, ty) ->
  ...
  | DUserTy (id, sizes, ty) ->
      Printf.sprintf "type %s%s = %s" (id_to_string id) (sizes_to_string sizes)
        (ty_to_string ty)
  | DConstr (id, ty, params, exp) ->
      Printf.sprintf "constr %s %s(%s) = %s;" (ty_to_string ty)
        (id_to_string id) (params_to_string params) (exp_to_string exp)
  | DModule (id, intf, ds) ->
      let intf_str =
        if List.is_empty intf then "" else " : " ^ interface_to_string intf
      in
      Printf.sprintf "module %s%s {\n%s\n}" (id_to_string id) intf_str
        (decls_to_string ds)
  | DModuleAlias (id1, e, cid1, cid2) ->
      Printf.sprintf "module %s = %s if %s else %s" (id_to_string id1)
        (cid_to_string cid1) (exp_to_string e) (cid_to_string cid2)
  | DAction (id, ret_tys, const_params, (dyn_params, acn_body)) ->
      Printf.sprintf "action (%s) %s(%s)(%s) {\n\taction_return (%s)\n}\n"
