...
type info = file_info StrMap.t

let global_info : info = StrMap.create 0
...
let read_one_file fname =
  ...
  let chan =
    try open_in fname
    with _ -> error (Printf.sprintf "file '%s' not found" fname)
  in
  try
    while true do
...
let read_file fname : unit = StrMap.add global_info fname (read_one_file fname)
...
let get_position_opt fname idx =
  if fname = "" then None
    ...
    let file_info = StrMap.find global_info fname in
    Array.iteri
      (fun i (s, e) ->
...
let get_position idx fname =
  ...
  | None ->
      failwith "internal error (get_position)"
  | Some x ->
      x
...
let show_message_position (span : Span.t) msg color label =
  ...
  | Some (l1, c1), Some (l2, c2) ->
      let file_info = StrMap.find global_info span.fname in
